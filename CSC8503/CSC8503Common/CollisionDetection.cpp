	...
	const CapsuleVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	float d0 = Vector3::Distance(aBottomSphere, bBottomSphere);
	float d1 = Vector3::Distance(aBottomSphere, bTopSphere);
	float d2 = Vector3::Distance(aTopSphere   , bBottomSphere);
	float d3 = Vector3::Distance(aTopSphere   , bTopSphere);
	...
	Transform tempA(worldTransformA);
	tempA.SetPosition(aLocalSphere);
	Transform tempB(worldTransformB);
	tempB.SetPosition(bLocalSphere);

	SphereVolume bSphereVolume(volumeB.GetRadius());
	SphereVolume aSphereVolume(volumeA.GetRadius());

	bool coll = SphereIntersection(aSphereVolume, tempA, bSphereVolume, tempB, collisionInfo);
	collisionInfo.point.localA = Vector3();
	collisionInfo.point.localB = Vector3();
	return coll;
}
