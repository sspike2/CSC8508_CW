...
PhysicsObject::PhysicsObject(Transform* parentTransform, const CollisionVolume* parentVolume)
{
	...
	friction = 0.5f;
	useGravity = true;
	//shouldUseProjection = false;
	collType = CollisionType::Impulse;
}
...
void PhysicsObject::InitCapsuleIntertia(float radiusSqr, float height)
{
	float z = 0.5f * inverseMass * (radiusSqr);
	float x, y;
	x = y = (1.0f / 12.0f) * inverseMass * ((3 * radiusSqr) + (height * height));
	inverseInertia = Vector3(x, y, z);
}
