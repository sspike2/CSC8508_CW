	...
	for (auto i = first; i != last; ++i)
	{
		...
		for (auto j = i + 1; j != last; ++j)
		{
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info))
			{
				...
				if ((*i)->GetPhysicsObject()->GetCollisionType() == CollisionType::Projection 
					|| (*j)->GetPhysicsObject()->GetCollisionType() == CollisionType::Projection)
				{
					ProjectionResolveCollision(*info.a, *info.b, info.point);
				}
				else if ((*i)->GetPhysicsObject()->GetCollisionType() == CollisionType::Penalty ||
					     (*j)->GetPhysicsObject()->GetCollisionType() == CollisionType::Penalty)
				{
...
void PhysicsSystem::PenaltyResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	float springMean = (physA->GetElasticity() + physB->GetElasticity()) / 2;
	float force = -springMean * p.penetration;
